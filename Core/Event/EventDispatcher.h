...
template<typename Type>
std::function<void(std::shared_ptr<IEventData>)> MakeDelegate(FuncPtr<Type> callback, Type* owner)
{
	return std::bind(callback, owner,std::placeholders::_1);
}

template<typename Type>
inline void RegisterToEvent(Type* owner, FuncPtr<Type> callback, EEventType eventType)
{
	EventDispatcher::GetInstance().AddListener(MakeDelegate(callback,owner),eventType);
}

template<typename Type>
inline void UnRegisterToEvent(Type* owner, FuncPtr<Type> callback, EEventType eventType)
{
	EventDispatcher::GetInstance().RemoveListener(MakeDelegate(callback, owner), eventType);
}