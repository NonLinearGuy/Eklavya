...
bool ShadowPassFB::Setup()
{
	...
	{
		LOG("Main Pass FB error : depth size is invalid");
		return false;
	}
	...
	{
		LOG("ERROR:: SHADOW PASS FRAMEBUFFER:: Framebuffer is not complete!");
		return false;
	}
...
bool GeneralFB::Setup()
{
	...
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
	{
		LOG("ERROR:: MAIN PASS FRAMEBUFFER:: Framebuffer is not complete!");
		return false;
	}
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
